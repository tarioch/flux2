...
import (
	...
	"github.com/fluxcd/flux2/pkg/manifestgen/deploysecret"
	"github.com/fluxcd/flux2/pkg/manifestgen/install"
	"github.com/fluxcd/flux2/pkg/manifestgen/kustomize"
...
func (b *Bootstrap) Reconcile(ctx context.Context, installOpts install.Options, deployOpts deploysecret.Options,
...
func (b *Bootstrap) DeployKey(ctx context.Context, repo gitprovider.UserRepository,
	options deploysecret.Options) error {
	...
	manifest, err := deploysecret.Generate(options)
	if err != nil {
		return err
	}
	...
	// Upsert private key
	privateKey, ok := secret.StringData[deploysecret.PrivateKeySecretKey]
	if ok {
		if repo != nil {
		...
		b.log.Successf("public key: %s", strings.TrimSpace(secret.StringData[deploysecret.PublicKeySecretKey]))
	}
...
func (b *Bootstrap) SyncConfig(ctx context.Context, url, branch string, options sync.Options) error {
	...
	kusManifests, err := kustomize.GenerateKustomizationYAML(filesys.MakeFsOnDisk(), filepath.Dir(manifests.Path),
		kustomize.Options{BaseDir: b.git.Path()})
	if err != nil {
